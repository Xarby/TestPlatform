...
gen_key_into_file(const char *public_key_file, const char *private_key_file, const char *machine_info_file)
{
    ...
    if (!fp)
        return -1;
    size_t info_len = fread(machine_info_buf, sizeof(char), MAX_INFO_LEN, fp);
    fclose(fp);
    if (info_len <= 0)
        return -1;
    ...
    // b) read public key
    uint8_t public_key_buf[MAX_BUF_LEN] = {0};
    fp = fopen(public_key_file, "r");
    if (!fp) {
        fprintf(stderr, "open orignal pub ke failed\n");
        return -1;
    }
    size_t public_key_size = fread(public_key_buf, sizeof(uint8_t), MAX_BUF_LEN, fp);
    fclose(fp);
    if (public_key_buf <= 0) {
